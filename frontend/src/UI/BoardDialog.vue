  ...
  <teleport to="body">
    <transition name="fade">
      ...
      <div v-show="modelValue" class="backdrop" @click.self="close">
        <div class="dialog">
          <div>
            <p class="addBoard">Add New Board</p>
            <div class="title">
            ...
            <p class="column">Columns</p>
            <div
              class="controls"
              v-for="(column, idx) in newBoardData.columns"
              :key="idx"
            >
...
const emit = defineEmits(["update:modelValue", "create-new-board"]);

const newBoardData = ref({
  name: "",
  columns: [
    {
      name: "Todo",
...
function addColumn() {
  newBoardData.value.columns.push({ name: "", tasks: [] });
}

function removeColumns(index) {
  newBoardData.value.columns.splice(index, 1);
}

function createBoard() {
  if (!newBoardData.value.name.trim()) {
    alert("Name cannot be empty!");
    return;
  }

  if (newBoardData.value.columns.some((col) => !col.name.trim())) {
    alert("Column names cannot be empty!");
    return;
  }

  emit("create-new-board", newBoardData.value);
  close();
}
